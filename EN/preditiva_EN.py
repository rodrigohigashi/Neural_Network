import pandas as pd
import numpy as np
from IPython.display import display
import pandas_profiling 
import sweetviz as sv

import matplotlib
import matplotlib.pyplot as plt
import seaborn as sns

# Performance Metrics
from sklearn.metrics import accuracy_score
from sklearn.metrics import roc_auc_score
from sklearn.metrics import precision_score
from sklearn.metrics import recall_score
from sklearn.metrics import f1_score
from scipy.stats import ks_2samp


def generate_eda_reports(df, target_feat, 
                        html_pp='base_eda_pp.html', 
                        html_sv='base_eda_sv.html'):
    '''
    '''
    # Generate report using Pandas Profiling
    perfil_pp = df.profile_report()
    perfil_pp.to_file(output_file=html_pp)
    
    # Gera report using SweetViz
    perfil_sv = sv.analyze(df, target_feat=target_feat)
    perfil_sv.show_html(html_sv)
    
    return perfil_pp, perfil_sv
    

class analise_iv:
        
    # private function
    def __get_tab_bivariate(self, chosen_var):
     
        # Creates the count of Target_1 and Target_0
        df_aux = self.df.copy() 
        df_aux['target2'] = self.df[self.target]
        df2 = df_aux.pivot_table(values='target2',
                                 index=chosen_var,
                                 columns=self.target,
                                 aggfunc='count')
        
        df2 = df2.rename(columns={0:'#Target_0',
                                  1:'#Target_1'})
        df2.fillna(0, inplace=True)

        # Creates the remaining columns of the bivariate table
        df2['Total'] = (df2['#Target_0'] + df2['#Target_1'])
        df2['%Freq'] = (df2['Total'] / (df2['Total'].sum()) * 100).round(decimals=2)
        df2['%Target_1'] = (df2['#Target_1'] / (df2['#Target_1'].sum()) * 100).round(decimals=2)
        df2['%Target_0'] = (df2['#Target_0'] / (df2['#Target_0'].sum()) * 100).round(decimals=2)
        df2['%Target_0'] = df2['%Target_0'].apply(lambda x: 0.01 if x == 0 else x) #fixes indeterminate log issue
        df2['%Target_1_Rate'] = (df2['#Target_1'] / df2['Total'] * 100).round(decimals=2)
        df2['Odds'] = (df2['%Target_1'] / df2['%Target_0']).round(decimals=2)
        df2['Odds'] = df2.Odds.apply(lambda x: 0.01 if x == 0 else x) #fixes indeterminate log issue
        df2['LN(Odds)'] = np.log(df2['Odds']).round(decimals=2)
        df2['IV'] = (((df2['%Target_1'] / 100 - df2['%Target_0'] / 100) * df2['LN(Odds)'])).round(decimals=2)
        df2['IV'] = np.where(df2['Odds'] == 0.01, 0 , df2['IV']) 

        df2 = df2.reset_index()
        df2['Variable'] = chosen_var
        df2 = df2.rename(columns={chosen_var: 'Var_Range'})
        df2 = df2[['Variable','Var_Range', '#Target_1','#Target_0', 'Total', '%Freq', '%Target_1', '%Target_0',
       '%Target_1_Rate', 'Odds', 'LN(Odds)', 'IV']]
        
        # Stores a copy of the table in the IV tracking history
        self.df_tabs_iv = pd.concat([self.df_tabs_iv, df2], axis = 0)
        
        return df2
        
    def get_bivariate(self, chosen_var='all_vars'):
        
        if chosen_var == 'all_vars':
                       
            #vars = self.df.drop(self.target,axis = 1).columns
            vars = self.get_list_iv().index
            for var in vars:
                tabela = self.df_tabs_iv[self.df_tabs_iv['Variable'] == var]
                print('==> "{}" tem IV de {}'.format(var,self.df_tabs_iv[self.df_tabs_iv['Variable'] == var]['IV'].sum().round(decimals=2)))
                # Displays the table in Jupyter notebook
                display(tabela)
            
            return
        
        else:
            print('==> "{}" tem IV de {}'.format(chosen_var,self.df_tabs_iv[self.df_tabs_iv['Variable'] == chosen_var]['IV'].sum().round(decimals=2)))
            return self.df_tabs_iv[self.df_tabs_iv['Variable'] == chosen_var]
                   
            
    def get_list_iv(self):
        
    
        # groups the list of IV's in descending order
        list = (self.df_tabs_iv.groupby('Variable').agg({'IV':'sum'})).sort_values(by=['IV'],ascending=False)
            
        return list
    
    

    def __init__(self, df, target, nbins=10):

        self.df = df.copy()
        self.target = target

        #numerical features list
        df_num = self.df.loc[:,((self.df.dtypes == 'int32') | 
                                (self.df.dtypes == 'int64') | 
                                (self.df.dtypes == 'float64')
                               )
                            ]

        vars = df_num.drop(target,axis = 1).columns

        for var in vars:
            nome_var = 'fx_' + var 
            df_num[nome_var] = pd.qcut(df_num[var], 
                                       q=nbins, 
                                       precision=2,
                                       duplicates='drop')
            df_num = df_num.drop(var, axis = 1)
            df_num = df_num.rename(columns={nome_var: var})

        #categorical features list
        df_str = self.df.loc[:,((self.df.dtypes == 'object') | 
                                (self.df.dtypes == 'category') |
                                (self.df.dtypes == 'bool'))]


        self.df = pd.concat([df_num,df_str],axis = 1)


         # inicialize historical tab
        self.df_tabs_iv = pd.DataFrame()

        vars = self.df.drop(self.target,axis = 1).columns
        for var in vars:
            self.__get_tab_bivariate(var);

        # remove tabs from iv duplicated
        self.df_tabs_iv = self.df_tabs_iv.drop_duplicates(subset=['Variable','Var_Range'], keep='last')
        
        
# KS calculating function
def ks_stat(y, y_pred):
    return ks_2samp(y_pred[y==1], y_pred[y!=1]).statistic

# Calculating models performance function
def calcula_desempenho(modelo, x_train, y_train, x_test, y_test):
    
    # Predicted value
    ypred_train = modelo.predict(x_train)
    ypred_proba_train = modelo.predict_proba(x_train)[:,1]

    ypred_test = modelo.predict(x_test)
    ypred_proba_test = modelo.predict_proba(x_test)[:,1]

    # Performance Metrics
    acc_train = accuracy_score(y_train, ypred_train)
    acc_test = accuracy_score(y_test, ypred_test)
    
    roc_train = roc_auc_score(y_train, ypred_proba_train)
    roc_test = roc_auc_score(y_test, ypred_proba_test)
    
    ks_train = ks_stat(y_train, ypred_proba_train)
    ks_test = ks_stat(y_test, ypred_proba_test)
    
    prec_train = precision_score(y_train, ypred_train, zero_division=0)
    prec_test = precision_score(y_test, ypred_test, zero_division=0)
    
    recl_train = recall_score(y_train, ypred_train)
    recl_test = recall_score(y_test, ypred_test)
    
    f1_train = f1_score(y_train, ypred_train)
    f1_test = f1_score(y_test, ypred_test)

    df_perf = pd.DataFrame({'Train':[acc_train, roc_train, ks_train, 
                                        prec_train, recl_train, f1_train],
                              'Test':[acc_test, roc_test, ks_test,
                                       prec_test, recl_test, f1_test]},
                            index=['Accuracy','AUROC','KS',
                                   'Precision','Recall','F1'])
    
    df_perf['Variation'] = round(df_perf['Test'] / df_perf['Train'] - 1, 2)
    
    return df_perf


# Create a function that returns the bivariate table of frequencies by variable
def tabela_bivariate(data,var):
    
    df = pd.DataFrame(data[var].value_counts()).sort_values(by=var,ascending=False)
    total = df[var].sum()
    df['Relative_Freq'] = (df[var]/total).round(decimals=2)
    df['Accumulated_Freq'] = df['Relative_Freq'].cumsum().round(decimals=2)
    return df;


def create_categorical_var_graph(tab):

    # Increase the size of the chart (width 8 and height 4)
    fig = plt.figure(figsize=(8,4))

    # Creates a bar chart using the table index as X-axis labels
    cor = np.random.rand(3)
    plt.bar(tab.index,tab['Relative_Freq'],width = 0.7, tick_label=tab.index,color=cor,alpha=0.6)

    plt.ylim(0,tab['Relative_Freq'].max()+0.2)
    plt.title("Relative Frequency of {}".format(list(tab.columns)[0]))

    # creates a set of pairs of labels and relative frequencies
    for x,y in zip(tab.index,tab['Relative_Freq']):

        # formats the percentage label
        rotulo = "{:.2f}".format(y)

        # places the label in position (x,y), aligned to the center and at a distance of 0.5 from the point (x,y)
        plt.annotate(rotulo,(x,y),ha='center',textcoords="offset points",xytext=(0,5))
        
        
def scatter_model(y_obs, y_pred):
    matplotlib.use('module://ipykernel.pylab.backend_inline')

    plt.style.use('ggplot')
    plt.rc('xtick', labelsize=10) 
    plt.rc('ytick', labelsize=10) 
    
    fig, ax = plt.subplots(figsize=(6, 6))
    
    min_val = min([np.min(y_obs.min()),y_pred.max()])*0.5
    max_val = max([np.max(y_obs.max()),y_pred.max()])*1.1
    
    plt.plot(y_obs, y_pred, 'ro')
    plt.xlabel('Observed', fontsize = 10)
    plt.ylabel('Predicted', fontsize = 10)    
    plt.title('Predicted vs. Observed', fontsize = 10)
    ax.plot([min_val, max_val], [min_val, max_val], 'k--', lw=1)
    ax.set_xlim([min_val, max_val])
    ax.set_ylim([min_val, max_val])
    plt.show()